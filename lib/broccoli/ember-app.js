...
class EmberApp {
  ...
  */
  ...
  getExtractedTemplates() {
    if (process.env.PREBUILD && !this.project.isEmberCLIAddon()) {
      let addonTrees = this.addonTreesFor('extractedTemplates');
      let mergedTemplates = mergeTrees(addonTrees, {
        overwrite: true,
        annotation: 'TreeMerger (extractedTemplates)',
      });

      let extractedTemplates = new Funnel(mergedTemplates, {
        srcDir: '/',
        destDir: `${this.name}/templates`,
        annotation: 'ProcessedTemplateTree',
      });
