...
const AddonDiscovery = require('./addon-discovery');
const AddonsFactory = require('./addons-factory');
...
let addonProto = {
  ...
  */
  treeFor(treeType) {
    ...
    const TREES_TO_NOT_PREBUILD = ['app', 'styles', 'public', 'test-support'];
    // This block is to return the prebuilt directory of an addon for a specific target group if its exists
    if (!TREES_TO_NOT_PREBUILD.includes(treeType) && this._checkIfPrebuilt()) {
      // Find the prebuilt directory from the target group and addon version
      const targetGrpDir = this.findChecksumOfTargetGroup();
      const addonVersion = this.pkg.version ? this.pkg.version : '';
      const prebuiltTree =  path.join(this.root, 'pre-built', `${targetGrpDir}-${addonVersion}`, treeType);

      // Check if prebuilt directory for the target group exists
      if(fs.existsSync(prebuiltTree)) {
        logger.info(`Returning ${treeType} tree for ${this.name}`);
        console.log(`**** Returning ${treeType} tree for ${this.name}`);
        if (cacheKeyForTreeType) {
          ADDON_TREE_CACHE.setItem(cacheKeyForTreeType, prebuiltTree);
        }
        node.stop();
        return prebuiltTree;
      }
  ...
  /**
   * Read targets of app and find md5 checksum to determine the prebuilt directory of an addon for the target group
   */
  findChecksumOfTargetGroup() {
    return md5Hex(this.project.targets.toString());
  },

 /**
 * Checks if an addon could be prebuilt, if the addon is not being developed and the package.json of the addon
 * specifies the trees that can be prebuilt
 * @param {*} prebuiltTree - path for the prebuilt directory of a tree in disc
 */
  _checkIfPrebuilt() {
    return this.pkg.prebuildTrees!== undefined && !this.isDevelopingAddon();
  },
...
Addon.resolvePath = function(addon) {
  let addonMain = (addon.pkg['ember-addon'] && addon.pkg['ember-addon'].main) || addon.pkg['main'] || 'index.js';
