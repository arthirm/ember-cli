...
let addonProto = {
  ...
  */
  treeFor(treeType) {
    ...
    let prebuiltTree =  path.join(this.root, 'pre-built', treeType);

    // If a tree is prebuilt, return the prebuilt tree
    if (!TREES_TO_NOT_PREBUILD.includes(treeType) && this._checkIfPrebuilt(prebuiltTree)) {
  ...
  /**
  * Checks if an addon is prebuilt
  * @param {*} prebuiltTree - path for the prebuilt directory of a tree in disc
  */
  _checkIfPrebuilt(prebuiltTree) {
    return this.pkg.prebuildTrees!== undefined && !this.isDevelopingAddon() && fs.existsSync(prebuiltTree)
  },
