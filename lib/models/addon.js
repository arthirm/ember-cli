...
const prebuildAddonUtils = require('../utilities/prebuild/prebuild-addon-utils');
...
let DEFAULT_TREE_FOR_METHODS = {
  ...
  extractedTemplates: 'treeForExtractedTemplates'
};
...
let addonProto = {
  ...
  */
  init(parent, project) {
    ...
    this.treePaths = {
      ...
      extractedTemplates: 'extracted-templates/templates',
      addon: 'addon',
  ...
  */
  treeFor(treeType) {
    ...
    // This block returns the prebuilt addon if it exists.
    // This feature is enabled if the environment variable PREBUILD is set
    if (process.env.PREBUILD) {
      let prebuiltTreeForAddon = prebuildAddonUtils.getPrebuiltTreeForAddon(this, treeType);
      if (prebuiltTreeForAddon) {
        // If prebuilt tree is present add it in the cache
        if (cacheKeyForTreeType) {
          ADDON_TREE_CACHE.setItem(cacheKeyForTreeType, prebuiltTreeForAddon);
        }
        node.stop();
        return prebuiltTreeForAddon;
      }
    }
    ...
    // This stores the addon build in prebuild directory if the prebuild feature is enabled
    if (process.env.PREBUILD && !mergeTrees.isEmptyTree(mergedTreesForType)) {
      mergedTreesForType = prebuildAddonUtils.storePrebuild(this, treeType, mergedTreesForType);
    }
    node.stop();
  ...
  */
  ...
  treeForExtractedTemplates(tree) {
    return tree;
  },
